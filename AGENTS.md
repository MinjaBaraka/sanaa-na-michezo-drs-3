# Sanaa na Michezo — ADT Bundle Reference
...
- TTS narration and sign-language video are independent and must be able to play simultaneously, regardless of which is started first.
- Playing, pausing, stopping, or skipping TTS must not pause, restart, seek, hide, or close the sign-language video.
- Playing, pausing, seeking, or closing the sign-language video must not interrupt or restart TTS.
- Keep sign-language videos silent while preserving their original visual background, including the green background. Removing background noise means removing audio, not replacing the background color.
- When changing media playback, verify both starting orders and each player's pause, stop, and close controls in a real browser. Do not introduce a shared exclusive playback mode or suppress intentional pause/close actions.

## About This Book

**Sanaa na Michezo** — Kitabu hiki cha Sanaa na Michezo ni kitabu cha mwanafunzi wa Darasa la Tatu kinachofundisha kwa Kiswahili mada za uigizaji, uimbaji, ala za muziki, uchoraji, ufinyanzi, mazoezi ya viungo, michezo ya asili na ushiriki katika maonesho na mashindano. Kimeandaliwa na Taasisi ya Elimu Tanzania kwa muktadha wa Tanzania Bara, hivyo kinaelekea wanafunzi wa shule ya msingi ya awali wenye umri mdogo. Kwa jumla, kitabu kinatumia mbinu za vitendo, picha, kazi na mazoezi ili kukuza ubunifu, vipaji, stadi za maisha, afya ya mwili na kuthamini utamaduni wa jamii.
...
├── SANAA-NA-MICHEZO-DRS-3.db                    # SQLite database (pipeline state)
├── SANAA-NA-MICHEZO-DRS-3.pdf                   # Original source PDF
